...
public class CalSalary_Board{
   private static CalSalary_Board calsalary_board = new CalSalary_Board();

   public static CalSalary_Board getCalSalary_Board() {
      return calsalary_board;
   }
   ...
   public String select(String id) {
      try {
		...
		/**************매개변수 id(휴대폰번호)를 이용해서 EMP_NO 출력*******************/
	     ...
	     query = "SELECT KIND_DAY FROM WORKTIME where EMP_NO ='"+emp_id+"' AND to_char(A_TIME, 'YYYY/MM')='"+nowTime.substring(0, 7)+"' and E_time IS NOT NULL";
			pstmt = conn.prepareStatement(query);
	        rs = pstmt.executeQuery();
	        while(rs.next()) {
		...
		query = "SELECT COUNT(WORKTIME_ID)WORKED_DAY FROM WORKTIME where EMP_NO ='"+emp_id+"' AND to_char(A_TIME, 'YYYY/MM')='"+nowTime.substring(0, 7)+"' and E_time IS NOT NULL";
		pstmt = conn.prepareStatement(query);
        rs = pstmt.executeQuery();
        while(rs.next()) {
       ...
       i=0;
       query = "SELECT to_char(a_time, 'YYYYMMDDHH24mi')atime, to_char(e_time, 'YYYYMMDDHH24mi')etime FROM WORKTIME where EMP_NO ='"+emp_id+"' AND TO_CHAR(A_TIME,'YYYY/MM')='"+nowTime.substring(0,7)+"' and E_time IS NOT NULL order by worktime_id";
       pstmt = conn.prepareStatement(query);
       rs = pstmt.executeQuery();  
       while(rs.next()) {
      ...
      /*****************Date Format 지정 및 출퇴근 배열의 Date형, DateTime형(double) 초기화*******************/
       ...
       /**********기준 시급 대입***********/
        ...
        if(sIsMoreFive.length()>0) { //5인 이상 사업체일경우
	        ...
	        Date nowTimeDate = MMFormat.parse(nowTime.substring(5,7));
	        ...
	        rs = pstmt.executeQuery();
	        while(rs.next()) {
	        	sRestDay = rs.getString("rest_day").trim();
	        }
	        query = "SELECT count(worktime_id)count_rest FROM worktime where EMP_NO ='"+emp_id+"'and kind_day='"+sRestDay+"' and to_char(a_time, 'YYYY/MM')='"+nowTime.substring(0,7)+"' and E_time IS NOT NULL"; 
             ...
             restCount = Integer.parseInt(sRestCount);
		        ...
		        query = "SELECT worktime_id FROM worktime where EMP_NO ='"+emp_id+"' and kind_day='"+sRestDay+"'and to_char(a_time, 'YYYY/MM')='"+nowTime.substring(0,7)+"' and E_time IS NOT NULL"; 
	             ...
	             System.out.println();
	             	...
	             	for(i=0;i<restCount;i++) {
	                 	query = "SELECT TO_CHAR(a_time,'HH24mi')A_TIME, TO_CHAR(e_time,'HH24mi')E_TIME FROM worktime where EMP_NO ='"+emp_id+"' and worktime_id='"+workTimeId[i]+"'and to_char(a_time, 'YYYY/MM')='"+nowTime.substring(0,7)+"' and E_time IS NOT NULL";
	        ...
	        query = "SELECT count(holi_date)holi_count FROM holiday where to_char(holi_date,'MM')='"+nowTime.substring(5,7)+"'"; 		//현재 달 기준 사내 지정 휴일의 개수(근로자의 날 포함) 가져오기
			pstmt = conn.prepareStatement(query);
	        rs = pstmt.executeQuery();
	        while(rs.next()) {
	        ...
	        query = "SELECT to_char(holi_date,'MM/DD')holi_date FROM holiday where to_char(holi_date,'MM')='"+nowTime.substring(5,7)+"'";		//DB에서 사내 지정 휴일(근로자의 날 포함) 가져오기
			pstmt = conn.prepareStatement(query);
	        rs = pstmt.executeQuery();
	        while(rs.next()) {
	        ...
	        for(i=0;i<holiCount;i++) {
		        query = "SELECT worktime_id FROM worktime where EMP_NO ='"+emp_id+"'and to_char(a_time,'MM/DD')='"+holiDate[i]+"' and to_char(a_time,'YYYY/MM')='"+nowTime.substring(0,7)+"' and E_time IS NOT NULL"; 
        ...
        query = "SELECT kind_day FROM worktime w inner join employee e on w.EMP_NO = e.emp_no where to_char(a_time,'yyyy/mm')='"+nowTime.substring(0,7)+"' and e.emp_id ='"+id+"' and E_time IS NOT NULL order by worktime_id";
		pstmt = conn.prepareStatement(query);
        rs = pstmt.executeQuery();
        i = 0;
        while(rs.next()) {
        ...
        System.out.println("첫째 주 의무 근무일: "+firstWeekWorkDay);System.out.println("마지막 주 의무 근무일: "+lastWeekWorkDay);
        cal = Calendar.getInstance();
      ...
      for(i=0;i<6;i++) {
    	  System.out.println(weeklyRecordWorkDay[i]);
      }
        ...
        for(j=1;j<6;j++) {
        	...
        	for(i=0;i<absent.length;i++) {
                System.out.print("a"+i+":" +absent[i]+"\t");
                }
        	System.out.println("");
        	
        	
        	/*
        	 //absent[0] = firstWeekWorkDay.length()-weeklyRecordWorkDay[0].length(); 첫번째 주의 결근 횟수 계산
        	//if(absent[0]<0)absent[0]=0;
        	if(hireWorkDay.length()-weeklyRecordWorkDay[j].length()==0) absent[j]=0;			//마지막주가 아닌 근무요일 배열의 길이-실제 출근 요일 배열의 길이->같으면 개근
        	else if(lastWeekWorkDay.length()-weeklyRecordWorkDay[j].length()==0) absent[j]=0;	//마지막주의 근무요일 배열의 길이 - 실제 출근 요일 배열의 길이->같으면 개근
        	
        	else{																												 //개근이 아닐 경우
        		if(lastWeekWorkDay.substring(lastWeekWorkDay.length(), lastWeekWorkDay.length()) 								 //그 달의 마지막 날의 근무 요일과
        				== weeklyRecordWorkDay[j].substring(weeklyRecordWorkDay[j].length(), weeklyRecordWorkDay[j].length())) { //실제 근무일의 마지막 요일이 같으면
        			absent[j] =lastWeekWorkDay.length()-weeklyRecordWorkDay[j].length();										 //결석일 = 마지막 주의 근무요일의 배열의 길이 - 실제 근무 요일 배열의 길이  
        			if(absent[j]<0)absent[j]=0;
        		}
        		else {
        			absent[j] = hireWorkDay.length()-weeklyRecordWorkDay[j].length();	//다를 시 결석일 = (첫째,마지막 째 주가 아닌 주의)근무요일의 배열의 갈이 - 실제 근무 요일의 배열의 길이	
        			if(absent[j]<0)absent[j]=0;
        		}
        	}
        	*/
        ...
        query = "SELECT MIN(to_char(a_time,'DD'))xFirWorkedDay from worktime where EMP_NO='"+emp_id+"' and to_char(a_time,'YYYY/MM')='"+Integer.toString(Integer.parseInt(nowTime.substring(0,7).replace("/", ""))-1)+"' and E_time IS NOT NULL";
        ...
        query = "SELECT MIN(to_char(a_time,'DD'))firWorkedDay from worktime where EMP_NO='"+emp_id+"' and to_char(a_time,'YYYY/MM')='"+Integer.toString(Integer.parseInt(nowTime.substring(0,7).replace("/","")))+"' and E_time IS NOT NULL";
		pstmt = conn.prepareStatement(query);		//이번 달에 일을 한 첫 날의 날짜를 출력
        rs = pstmt.executeQuery(); 
        while(rs.next()) {
        ...
        for(j=0;j<6;j++) {
        	...
        	System.out.println(j+"쨰 주 근무 시간 :"+weekly_time+"\t");
       ...
       System.out.println("restDay:"+ restDay);
       System.out.println("holiDay:"+ holiday);
       System.out.println("deduct: " + deduct);

       returns = total+"-"+nowTime+"-"+basic+"-"+extend+"-"+night+"-"+weekly+"-"+restDay+"-"+holiday+"-"+deduct;
