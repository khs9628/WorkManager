            ...
            <!-- 출근중 -->
             <div class="row">
                          ...
                          <h4 class="pull-left"><span class="glyphicon glyphicon-tags"></span>&nbsp;&nbsp; 마이 페이지</h4>
                    		...
                    		<%
                           int STD_salary = memberHire.getSTD_Salary(); 
                    		String numStr2 = String.valueOf(STD_salary);
                            if(numStr2.equals("0")|| numStr2 ==null){
                           	 			
                           	 			%>
                    <button type="button" class="btn btn-danger pull-right" onclick = "location.href='memberDeleteServlet?EMP_NO=<%=memberHire.getEMP_NO()%>'">삭제</button>      
                    <button type="button" class="btn btn-success pull-right" onclick = "location.href='memberHireInfoPlus.jsp?EMP_NO=<%=memberHire.getEMP_NO()%>'">추가</button>
                    				<%
                    				} else{
                    				%>		  
                    				  <button type="button" class="btn btn-danger pull-right" onclick = "location.href='memberDeleteServlet?EMP_NO=<%=memberHire.getEMP_NO()%>'">삭제</button>
                         			  <button type="button" class="btn btn-success pull-right" onclick = "location.href='memberInfoUpdate.jsp?EMP_NO=<%=memberHire.getEMP_NO()%>'">수정</button>
                         <%
                    				}
                    				%>
                          </div>
                          ...
                          <div class="panel-body">
							...
							<p>	id(전화번호) : <%=memberHire.getEMP_ID() %><br> </p>
							<p>	email : <%=memberHire.getEMP_EMAIL() %><br> </p>
							<p>    사원번호 : <%=memberHire.getEMP_NO() %> <br> </p>
                              ...
                              <div class="well well-lg">
                         <p>    사원번호 : <%=memberHire.getEMP_NO() %><br> </p>
                          <p>    근무지명 : <%=memberHire.getWorkPlace() %><br> </p>
 						  <p> 급여종류 :  <%=memberHire.getSalType() %><br> </p>
                          <p> 기준급여 :  <%=memberHire.getSTD_Salary() %><br> </p>
                          <p> 주 근로일 : <%=memberHire.getWORK_DAY() %>
                  		  			주휴일:	<%=memberHire.getREST_DAY() %><br>  </p>
                  		  <p> 기준 근로시간 : <%=memberHire.getSTART_TIME() %> ~	<%=memberHire.getEND_TIME() %><br>  </p>
                  		  <p> 월급일 :  <%=memberHire.getSALDATE() %><br> </p>
           				  <p> 입사일 :  <%=memberHire.getJoinDate() %><br> </p>
           				  	 </div>
                             <div class= "container">
                       </div>
                              </div>
